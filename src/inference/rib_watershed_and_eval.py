...
def load_tiff_data(file_path: str):
    """
    Load data from a TIFF file using SimpleITK.
    
    Args:
        file_path: Path to the TIFF file.
    
    Returns:
        tuple: (data, sitk_image) - numpy array and SimpleITK image object
    """
    ...
    print(f"  Shape: {data.shape}, dtype: {data.dtype}, range: [{np.min(data):.4f}, {np.max(data):.4f}]")
...
def load_prediction_data_from_directory(input_dir: str, affinity_filename: str = "affinity_ch1.tif", 
                                       skeleton_filename: str = "skeleton_distance.tif",
                                       transpose_axes: bool = False):
    """
    Load prediction data from separate TIFF files in a directory.
    
    Args:
        input_dir: Path to the directory containing the TIFF files.
        affinity_filename: Name of the affinity channel file (default: "affinity_ch1.tif").
        skeleton_filename: Name of the skeleton channel file (default: "skeleton_distance.tif").
        transpose_axes: If True, transpose from (X,Y,Z) to (Z,Y,X) format (default: False).
    
    Returns:
        tuple: (affinity_channel, skeleton_channel, sitk.Image) - The loaded channels and SimpleITK image object (for metadata)
    """
    input_dir = Path(input_dir)
    
    if not input_dir.exists() or not input_dir.is_dir():
        raise ValueError(f"Input directory not found or is not a directory: {input_dir}")
    
    # Load affinity channel
    affinity_path = input_dir / affinity_filename
    if not affinity_path.exists():
        raise FileNotFoundError(f"Affinity file not found: {affinity_path}")
    
    affinity_channel, affinity_sitk = load_tiff_data(str(affinity_path))
    
    # Load skeleton channel
    skeleton_path = input_dir / skeleton_filename
    if not skeleton_path.exists():
        raise FileNotFoundError(f"Skeleton file not found: {skeleton_path}")
    
    skeleton_channel, skeleton_sitk = load_tiff_data(str(skeleton_path))
    
    # Check if shapes match
    if affinity_channel.shape != skeleton_channel.shape:
        raise ValueError(f"Shape mismatch: affinity {affinity_channel.shape} vs skeleton {skeleton_channel.shape}")
    
    # Transpose axes if requested (e.g., from (X,Y,Z) to (Z,Y,X))
    if transpose_axes:
        print(f"Transposing axes from {affinity_channel.shape} to match (Z,Y,X) format...")
        affinity_channel = np.transpose(affinity_channel, (2, 1, 0))
        skeleton_channel = np.transpose(skeleton_channel, (2, 1, 0))
        print(f"New shape after transpose: {affinity_channel.shape}")
        # Return None for sitk_image since the metadata won't match the transposed shape
        return affinity_channel, skeleton_channel, None
...
def apply_watershed_segmentation(affinity_channel, skeleton_data, binary_thr=0.5, skeleton_thr=0.0, min_size=100, 
    ...
    2. Use affinity channel to get foreground, then refine using initial instances as seeds
    
    Args:
        affinity_channel: affinity channel data for foreground detection
    ...
    # Step 2.1: Create foreground mask from affinity channel
    ...
    print(f"unique labels: {unique_labels[:10]}..., counts: {counts[:10]}...")
...
def process_rib_watershed_and_eval(
    ...
    """
    Load prediction data from directory with separate TIFF files, apply watershed segmentation, and evaluate.
    ...
    Args:
        ...
        skeleton_filename: Name of the skeleton channel file (default: "skeleton_distance.tif").
        transpose_axes: If True, transpose from (X,Y,Z) to (Z,Y,X) format (default: False).
        binary_threshold: Threshold for affinity channel to create foreground mask (default: 0.5).
        ...
        output_format: Output format: "tiff", "zarr", "h5", "nii.gz" (default: "tiff").
    ...
    print(f"\n=== Loading prediction data from directory: {input_dir} ===")
    affinity_channel, skeleton_channel, reference_sitk_image = load_prediction_data_from_directory(
        input_dir, 
        affinity_filename=affinity_filename,
        skeleton_filename=skeleton_filename,
        transpose_axes=transpose_axes
    ...
    print(f"\nAffinity channel shape: {affinity_channel.shape}, dtype: {affinity_channel.dtype}")
    ...
    if reference_sitk_image is None:
    ...
    # Save results
    # Save foreground mask (from affinity channel)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Apply watershed segmentation on RIB prediction data (separate TIFF files) and evaluate",
...
Examples:
  ...
  # Transpose axes to match ground truth format (if TIFF is (X,Y,Z) but GT is (Z,Y,X))
  python rib_watershed_and_eval.py --input_dir /path/to/data/ --output_path output_dir/ \\
      --gt_file ground_truth.nii.gz --transpose_axes
  
  # Specify custom filenames
  python rib_watershed_and_eval.py --input_dir /path/to/data/ --output_path output_dir/ \\
      --affinity_filename custom_affinity.tif --skeleton_filename custom_skeleton.tif
  
  # Use fast edt library with 8 parallel threads (5-10x faster)
  python rib_watershed_and_eval.py --input_dir /path/to/data/ --output_path output_dir/ \\
      --edt_parallel 8
  ...
  python rib_watershed_and_eval.py --input_dir /path/to/data/ --output_path output_dir/ \\
    ...
    parser.add_argument("--affinity_filename", type=str, default="affinity_ch1.tif", 
                       help="Name of the affinity channel file (default: affinity_ch1.tif)")
    parser.add_argument("--skeleton_filename", type=str, default="skeleton_distance.tif", 
                       help="Name of the skeleton channel file (default: skeleton_distance.tif)")
    parser.add_argument("--transpose_axes", action="store_true", default=False,
                       help="Transpose axes from (X,Y,Z) to (Z,Y,X) format to match ground truth (default: False)")
    
    # Watershed segmentation options
    parser.add_argument("--binary_threshold", type=float, default=0.5, 
                       help="[Stage 2] Threshold for affinity channel to create foreground mask (default: 0.5)")
    ...
    parser.add_argument("--output_format", type=str, default="tiff", 
                       choices=["zarr", "h5", "nii.gz", "tiff"],
                       help="Output format: 'zarr', 'h5', 'nii.gz', 'tiff' (default: tiff)")
